...
class DocumentExtTest extends AbstractMDOTest {
  DocumentExtTest() {
  ...
  void testEDT() {
    ...
    checkTemplates(mdo);
    checkCommands(mdo);
    checkAttributes(((MDObjectComplex) mdo).getAttributes().stream().filter(
    ...
    assertThat(((MDObjectComplex) mdo).getAttributes())
