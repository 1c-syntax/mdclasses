...
public class DesignConfiguration extends AbstractConfiguration {
  ...
  private void initializeProperties(Configuration configurationXML) {
    ...
    try {
      ...
      LOGGER.error("Не удалось получить значение CompatibilityMode. Причина " + e.getMessage());
    }
    ...
    try {
      ...
      LOGGER.error("Не удалось получить значение ScriptVariant. Причина " + e.getMessage());
    }
