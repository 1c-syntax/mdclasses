...
public class DesignerPaths {
   ...
   */
  public final String CONFIGURATION_MDO_PATH = "Configuration.xml";
  /**
   ...
   */
  public Path formDataPath(Path path, @NonNull String name) {
    return dataPath(name, path, "Form.xml");
  }
  ...
  public Path mdoPath(@NonNull Path folder, @NonNull String name) {
    return Paths.get(folder.toString(), name + EXTENSION_DOT);
  }
   ...
   */
  public Path rootPathByMDO(@NonNull Path mdoPath) {
  ...
  public Path parentConfigurationsPath(@NonNull Path rootPath) {
    return Paths.get(rootPath.toString(), EXT_DIR_NAME, PARENT_CONFIGURATIONS_BIN_NAME);
  }
